...
interface StoryState {
  scene: SceneId;
  lastStoryScene: SceneId;
  load: number;
  vibeShift: number;
  distanceTraveled: number;
  activeDeepDive: DeepDive | null;
  activeResearchExcerpt: ResearchExcerpt | null;
...
const STORY_BEATS: Record<SceneId, StoryBeat> = {
  prologue: {
    ...
    text: "The city got two faces. One it shows you at noon—clean teeth, tourist smile, sunscreen and brunch. The other shows up around 3 a.m.—makeup smeared, neon flickering, truth leaking through the cracks like cheap tequila. I’ve slept through both. — Marcus, notebook, Day 9\n\nAuthor’s note: Marcus is a composite. The road is real. The observations come from lived ground-level experience. Some details are reshaped for narrative clarity. The sociology underneath this story is explored more formally in the [[bwb-theory|companion academic paper]]. This version just shows the bones.",
  ...
  'chapter-3': {
    ...
    deepDives: [
      ...
      },
      {
        ...
        content: "The ANCODI complex (Anger, Contempt, Disgust) is a psychological profile used to track the escalation of group antagonism toward a perceived outsider."
      }
    ]
  },
  'chapter-4': {
    ...
    deepDives: [
      {
        ...
        icon: <Layers size={16} />,
        content: "The folk devil position constitutes a form of involuntary structural labor whose costs are borne entirely by the individual while the benefits accrue to the community."
      }
    ]
  },
  'chapter-6': {
  ...
  'academic-paper': {
    ...
    location: "Archives",
    text: "Accessing the full research library...",
    marginalia: "Formal sociology.",
    nextScene: 'prologue',
    deepDives: []
  }
};
...
export default function App() {
  const [state, setState] = useState<StoryState>({
    scene: 'prologue',
    lastStoryScene: 'prologue',
    load: 31,
    vibeShift: 0,
    distanceTraveled: 0,
    activeDeepDive: null,
    activeResearchExcerpt: null,
  ...
  const handleResearchLink = (paperId: string, sectionId?: string) => {
    ...
    if (sectionId) {
      ...
      if (section) {
        content = section.content[0]; // Show first paragraph as excerpt
        title = section.title;
      }
    }
  ...
  const renderTextWithLinks = (text: string) => {
    ...
    return parts.map((part, index) => {
      ...
      if (match) {
        const [full, linkData, linkText] = match;
        const [paperId, sectionId] = linkData.split('#');
        return (
          <button
            key={index}
            onClick={() => handleResearchLink(paperId, sectionId)}
            className="text-[#8b2e0f] border-b border-[#8b2e0f]/30 hover:border-[#8b2e0f] transition-all font-serif italic cursor-pointer inline-block"
          >
            {linkText}
          </button>
        );
  ...
  const handleNext = () => {
    ...
    setState(prev => ({
      ...prev,
      scene: nextId,
      lastStoryScene: nextId !== 'academic-paper' ? nextId : prev.lastStoryScene,
    ...
    <div className="min-h-screen bg-stone-950 flex items-center justify-center p-4 md:p-8 font-serif selection:bg-stone-200 selection:text-stone-900">
      ...
      <AnimatePresence mode="wait">
        {state.scene === 'academic-paper' ? (
          <AcademicPaper 
            key="academic-paper-view"
            onBack={() => setState(s => ({ ...s, scene: s.lastStoryScene }))}
          ...
          <motion.main 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="relative w-full max-w-4xl bg-[#fffdfa] min-h-[85vh] shadow-[0_0_100px_rgba(0,0,0,0.5)] rounded-sm flex flex-col md:flex-row overflow-hidden"
          >
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <div className="relative">
                      <div className="text-xl md:text-2xl leading-relaxed text-stone-800 font-serif first-letter:text-6xl first-letter:font-bold first-letter:mr-3 first-letter:float-left first-letter:text-stone-900 whitespace-pre-wrap">
                        {renderTextWithLinks(currentBeat.text)}
                      </div>
                    </div>
      ...
      {/* Research Excerpt Modal */}
      <AnimatePresence>
        {state.activeResearchExcerpt && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[80] flex items-center justify-center p-6 bg-stone-950/80 backdrop-blur-md"
            onClick={() => setState(s => ({ ...s, activeResearchExcerpt: null }))}
          >
            <motion.div 
              initial={{ scale: 0.9, opacity: 0, y: 20 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.9, opacity: 0, y: 20 }}
              className="bg-[#f0ebe2] border-2 border-[#8b2e0f]/20 p-10 rounded-sm max-w-xl w-full shadow-[0_20px_50px_rgba(0,0,0,0.5)] space-y-8 relative overflow-hidden"
              onClick={e => e.stopPropagation()}
            >
              {/* Decorative Elements */}
              <div className="absolute top-0 left-0 w-full h-1 bg-[#8b2e0f]" />
              <div className="absolute -right-12 -top-12 w-24 h-24 bg-[#8b2e0f]/5 rounded-full" />
              ...
              <div className="flex items-center justify-between border-b border-stone-300 pb-4">
                <div className="flex items-center gap-3 text-[#8b2e0f]">
                  <FileText size={16} />
                  <span className="font-['Space_Mono'] text-[0.6rem] uppercase tracking-[0.2em]">Research Excerpt</span>
                </div>
                <button 
                  onClick={() => setState(s => ({ ...s, activeResearchExcerpt: null }))} 
              ...
              <div className="space-y-2">
                <h4 className="font-['Space_Mono'] text-[0.55rem] uppercase tracking-widest text-stone-400">
                  From: {state.activeResearchExcerpt.paperTitle}
                </h4>
                <h3 className="text-2xl font-serif text-stone-900 leading-tight italic">
                  {state.activeResearchExcerpt.title}
                </h3>
              </div>

              <div className="relative">
                <div className="absolute -left-4 top-0 bottom-0 w-0.5 bg-[#8b2e0f]/20" />
                <p className="text-stone-800 leading-relaxed font-serif text-lg italic pl-4">
                  "{state.activeResearchExcerpt.content}"
                </p>
              </div>
              ...
              <div className="pt-6 flex flex-col gap-3">
                <button 
                  onClick={() => {
                    const id = state.activeResearchExcerpt?.paperId;
                    setState(s => ({ ...s, scene: 'academic-paper', selectedPaperId: id || null, activeResearchExcerpt: null }));
                  }}
                  className="w-full py-3 bg-[#8b2e0f] text-white font-serif italic text-sm hover:bg-[#a03a1a] transition-all rounded-sm flex items-center justify-center gap-2"
                >
                  Read Full Paper
                </button>
                <button 
                  onClick={() => setState(s => ({ ...s, activeResearchExcerpt: null }))}
                  className="w-full py-3 border border-stone-300 text-stone-600 font-serif italic text-xs hover:bg-stone-100 transition-all rounded-sm"
                >
                  Dismiss
                </button>
              </div>
